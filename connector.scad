...
module connector(top = 7, diam = 3, pinWidth = 4, plugDiam = 6, tolerance = 0) {
    union() {
        hull() {
            translate([(pinWidth + tolerance*2)/2 + 0.25,0,(top - tolerance) - (plugDiam - tolerance*2)/2])
            rotate([0,90,0])
            cylinder(h = pinWidth + tolerance*2 + 0.5, d = diam - tolerance*2, center = true, $fn = 25);
        ...
        hull() {
            ...
            rotate([0,90,0])
            cylinder(h = pinWidth/2 - tolerance*2, d = plugDiam - tolerance*2, center = true, $fn = 25);
